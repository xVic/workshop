...
[source,shell]
----
$ curl -X POST -d  '{"level":5, "name":"Chewbacca", "powers":"Agility, Longevity"}'  -H "Content-Type: application/json" $(oc get route -n USERNAME-codeready | grep 8083 | awk '{ print $2 }')/api/heroes -v
